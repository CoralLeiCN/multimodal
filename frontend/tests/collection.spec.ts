...
test("browses, searches through the dedicated API, and opens attribution", async ({ page }) => {
  ...
  const query = page.getByLabel("Describe an object")
  const suggestions = ["a brass microscope", "an early computer"]
  const prefilled = await query.inputValue()
  expect(suggestions).toContain(prefilled)
  await expect(query).toHaveCSS("color", "rgb(142, 147, 135)")
  await page.getByRole("button", { name: "Explore", exact: true }).click()
  await expect(page.getByRole("heading", { name: "Your discoveries" })).toBeVisible()
  expect(received).toBe(prefilled)
  await query.click()
  ...
  await expect(query).toHaveCSS("color", "rgb(48, 56, 44)")
  await query.press("Tab")
  await query.focus()
  await expect(query).toHaveValue("brass microscope")
  await page.getByRole("button", { name: "Explore", exact: true }).click()
  await expect(page.getByRole("heading", { name: "Your discoveries" })).toBeVisible()
  await expect.poll(() => received).toBe("brass microscope")
  await expect(page.getByTestId("image-card")).toHaveCount(1)
  ...
  await expect(page.getByTestId("image-card")).toHaveCount(2)
  expect(suggestions).toContain(await query.inputValue())
  await query.focus()
  await expect(query).toHaveValue("")
})
